...
const App = () => {
  ...
  // 분석 실행 (첨부 아이템 배열과 설명 수신)
  const runAnalysis = (items: any[], description: string) => {
    setCurrentScreen('ANALYZING');
    setTimeout(() => {
      const mockData: AnalysisData = {
        resultScore: Math.floor(Math.random() * 40) + 60,
        shareMe: 45,
        sharePartner: 55,
        replyTime: "15분",
        syncIndex: 82,
        keywords: ["오늘뭐해", "보고싶다", "웃겨", "카페"],
        mbti: "ENFP",
        attachment: "안정형",
        bigFive: { extraversion: 80, agreeableness: 85, conscientiousness: 60, openness: 90, neuroticism: 30 },
        moment: "대화 중간에 질문을 던진 시점부터 상대방의 반응이 눈에 띄게 좋아졌습니다.",
        tips: "상대방의 관심사에 더 집중해보세요.",
        warning: "너무 빠른 답장은 가끔 부담을 줄 수 있습니다."
      };
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case 'RESULT':
        return <ResultScreen resultData={analysisResult} onReset={() => setCurrentScreen('START')} />;
      case 'ARCHIVE':
        return (
          <ArchiveScreen 
