...
type Screen = 'START' | 'CATEGORY' | 'INPUT' | 'ANALYZING' | 'RESULT' | 'LOGIN' | 'ARCHIVE' | 'SETTINGS' | 'TRASH';
...
const App = () => {
  ...
  const runAnalysis = (imageUri: string, description: string) => {
    ...
    setTimeout(() => {
      const mockData: AnalysisData = {
        ...
        mbti: "ENFP",
        attachment: "안정형",
        bigFive: { extraversion: 80, agreeableness: 85, conscientiousness: 60, openness: 90, neuroticism: 30 },
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case 'START': return <StartScreen onStart={() => setCurrentScreen('CATEGORY')} onOpenMypage={() => setIsMypageOpen(true)} />;
      case 'CATEGORY': return <CategoryScreen onBack={() => setCurrentScreen('START')} onNext={() => setCurrentScreen('INPUT')} />;
      ...
      case 'RESULT': return <ResultScreen resultData={analysisResult} onReset={() => setCurrentScreen('START')} />;
      ...
      case 'TRASH':
        return (
          <TrashScreen
            ...
            onDeletePermanently={(id) => setTrashItems(trashItems.filter(i => i.id !== id))}
  ...
  return (
    <View style={{ flex: 1 }}>
      <SafeAreaView style={[
        styles.safeArea, 
        (currentScreen === 'START' || currentScreen === 'CATEGORY' || currentScreen === 'LOGIN' || currentScreen === 'ARCHIVE' || currentScreen === 'SETTINGS' || currentScreen === 'TRASH') && { backgroundColor: '#E6E6FA' }
      ]}>
      ...
      <Mypage 
        ...
        userInfo={userInfo}
        onLoginPress={() => { setIsMypageOpen(false); setCurrentScreen('LOGIN'); }}
